...
CREATE TABLE IF NOT EXISTS `vehicles`.`vehicle` (
  ...
  INDEX `fk_vehicule_make_idx` (`make_idmake` ASC) VISIBLE,
  INDEX `fk_vehicule_model1_idx` (`model_idmodel` ASC) VISIBLE,
  CONSTRAINT `fk_vehicule_make`
    FOREIGN KEY (`make_idmake`)
    REFERENCES `vehicles`.`make` (`idmake`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_vehicule_model1`
...
CREATE TABLE IF NOT EXISTS `vehicles`.`inventory` (
  ...
  INDEX `fk_inventory_vehicule1_idx` (`vehicle_idvehicle` ASC) VISIBLE,
  CONSTRAINT `fk_inventory_color1`
    FOREIGN KEY (`color_idcolor`)
    REFERENCES `vehicles`.`color` (`idcolor`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_inventory_vehicule1`
...
CREATE TABLE IF NOT EXISTS `vehicles`.`vehicleincentive` (
  ...
  INDEX `fk_vehiculeincentive_vehicule1_idx` (`vehicle_idvehicle` ASC) VISIBLE,
  INDEX `fk_vehiculeincentive_incentive1_idx` (`incentive_idincentive` ASC) VISIBLE,
  CONSTRAINT `fk_vehiculeincentive_vehicule1`
    FOREIGN KEY (`vehicle_idvehicle`)
    REFERENCES `vehicles`.`vehicle` (`idvehicle`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_vehiculeincentive_incentive1`
